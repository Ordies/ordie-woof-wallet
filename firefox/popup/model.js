...
class Model {
    ...
    async requestPermissions() {
        await browser.permissions.request({ origins: ["*://dogechain.info/*", "*://dogecoin.earlyordies.com/*", "*://api.blockchair.com/*"] })
    ...
    async loadPermissions() {
        ...
        if (!permissions.origins.includes("*://dogecoin.earlyordies.com/*")) {
            this.hasAllPermissions = false
            return
        }
    ...
    async refreshUtxos() {
        ...
        const resp3 = await fetch(`https://dogecoin.earlyordies.com/block/${json2.hash}`)
        if (resp3.status != 200) throw new Error("dogecoin.earlyordies.com is out of sync")
    ...
    async refreshInscriptionIds() {
        ...
        for (const utxo of this.utxos) {
            ...
            if (!inscriptionIdsPerOutput[key]) {
                const resp = await fetch(`https://dogecoin.earlyordies.com/output/${utxo.txid}:${utxo.vout}`)
    ...
    async refreshInscriptionContent(inscriptionIds, inscriptionOutpoints) {
        ...
        for (let i = 0; i < keys.length; i++) {
            ...
            if (!inscriptions[key]) {
                ...
                const url = `https://dogecoin.earlyordies.com/content/${inscriptionId}`
                ...
                const url2 = `https://dogecoin.earlyordies.com/shibescription/${inscriptionId}`
